...
def launch_setup(context, *args, **kwargs):
    ...
    if LaunchConfiguration("use_agnocast").perform(context) == "1":
        # The executable runs an agnocast-only executor, which no component container
        # offers, so the node is started standalone and the container argument is unused.
        return [
            Node(
