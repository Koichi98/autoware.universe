...
def _use_agnocast():
    return os.environ.get("ENABLE_AGNOCAST", "0") == "1"
...
def launch_setup(context, *args, **kwargs):
    ...
    if _use_agnocast():
        # Composable node loading is not supported with Agnocast, so launch the
        # standalone executable generated by autoware_agnocast_wrapper_register_node.
        # The heaphook must be preloaded (ld_preload_value is provided by
...
def generate_launch_description():
    ...
    if _use_agnocast():
        # Resolves ld_preload_value (heaphook + existing LD_PRELOAD) and spawns the
        # per-launch Agnocast discovery agent.
        actions.append(
