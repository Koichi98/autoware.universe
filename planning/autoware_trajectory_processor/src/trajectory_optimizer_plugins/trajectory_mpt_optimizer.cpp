  ...
  const std::shared_ptr<autoware_utils_debug::TimeKeeper> & time_keeper)
{
  ...
  try {
    // Get vehicle info. Uses the node-type-agnostic BasicVehicleInfoUtils so it works with
    // the agnocast wrapper node (which has no underlying rclcpp::Node in agnocast mode).
    vehicle_info_ =
      autoware::vehicle_info_utils::BasicVehicleInfoUtils<autoware::agnocast_wrapper::Node>(
        *node_ptr)
        .getVehicleInfo();
