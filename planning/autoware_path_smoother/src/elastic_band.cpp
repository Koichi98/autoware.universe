...
// Explicit instantiations for the supported node types.
// When Agnocast is disabled, autoware::agnocast_wrapper::Node is just an alias of
// rclcpp::Node, so instantiating it again would be a duplicate; guard it accordingly.
...
// Explicit instantiations for the supported node types.
// See the note on declare_eb_param above regarding the USE_AGNOCAST_ENABLED guard.
